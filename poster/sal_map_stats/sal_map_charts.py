...
print(l2_dist(soy_maps, corn_maps, axis=(1,2,3)).shape, l1_dist(soy_maps, corn_maps), perc_dist(soy_maps, corn_maps))
...
fig, ax = plt.subplots()
ax.plot_date(dates, l2_dist(soy_maps, corn_maps, axis=(0,1,3)), '-', color='red', label='L2 Dist')
ax.plot_date(dates, l1_dist(soy_maps, corn_maps, axis=(0,1,3)), '-', color='blue', label='L1 Dist')
...
plt.bar(bands, l2_dist(soy_maps, corn_maps, axis=(0,1,2)), bar_width, color=PLOT_COLOR, alpha=0.75, label='L2 Dist')
plt.bar(bands + bar_width, l1_dist(soy_maps, corn_maps, axis=(0,1,2)), bar_width, alpha=0.75, label='L1 Dist')
plt.xlim(1,9)
plt.xlabel('Band', fontsize=24)
plt.ylabel('Average L2/L1 Difference', fontsize=24)
plt.title('Relative Importance of Bands', fontsize=26)
plt.legend()
plt.show()

plt.bar(np.arange(soy_maps.shape[1]), l2_dist(soy_maps, corn_maps, axis=(0,2,3)), color=PLOT_COLOR, alpha=0.75)
plt.show()

plt.hist(l2_dist(soy_maps, corn_maps, axis=(1,2,3)), bins=30, color=PLOT_COLOR, alpha=0.75)
plt.hist(l1_dist(soy_maps, corn_maps, axis=(1,2,3)), bins=30, color='red', alpha=0.75)
plt.show()