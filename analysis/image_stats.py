...
def gather_stats(pathname, raw_pathname, outpath, MAX_FILES):
    ...
    for filename in filenames_all:
        if 'zoom' not in filename and filename.endswith('.npy\n'):
            # print filename
            if filename[23] == '_':
                if int(filename[22:23]) in [5, 17, 18, 19, 20, 27, 29, 31, 38, 39, 46]:
                    filenames.append(filename)
            else:
                if int(filename[22:24]) in [5, 17, 18, 19, 20, 27, 29, 31, 38, 39, 46]:
                    filenames.append(filename)    
    if len(filenames) > MAX_FILES:
        print 'Randomizing'
        filenames = list(np.random.choice(filenames, MAX_FILES, replace=False))
